...
(function($) {
	...
	var defaults = {
		...
		structure: 	 			  '<span class="prev_button">prev</span>'
								...
								+ '<span class="info_slide"><div class="info_slide_cont"><div class="info_slide_cont_center"></div></div></span>'
	...
	$sk.fn.extend({
		...
		setup: function() 
		{
			...
			// AstoSoft
			//this.settings.width_skitter = 978;
			//this.settings.height_skitter = 531;
			...
			var addImageLink = function (link, src, animation_type, label, target) {
				...
				if (self.settings.thumbs) {
					...
					self.box_skitter.find('.info_slide_cont_center').append(
				...
				$.ajax({
					...
					success: function(xml) {
						...
						$(xml).find('skitter slide').each(function(){
							...
							// AstoSoft
							var src 			= $(this).find('image').attr('longdesc');
		...
		loadImages: function (start) 
		{
			...
			for (var u=start; u<total; u++) {
				var self_il = this.settings.images_links[u];
		...
		/*
		 * AstoSoft start
		 * Waiting for load next image
		 */
		check_image: function() {
			this.box_skitter.append('<div class="loading"><span class="image_loading" style="display: block;position: absolute, top: -9999em"></span></div>');
			var self = this;
			
			self.loadImages(self.settings.image_i);
			
			setTimeout(function () {	
				$('img[longdesc$="' + self.settings.images_links[self.settings.image_i][0] + '"]').each(function() {
					image = this;
					image.src = self.settings.images_links[self.settings.image_i][0]
				});
		...
		completeMove: function () 
		{
			this.clearTimer(true);
			this.box_skitter.find('.box_clone').remove();
			if (!this.settings.is_paused && !this.settings.is_blur) this.check_image();
		},
