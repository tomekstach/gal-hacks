...
class JSNUniformModelForm extends JModelItem
{
	...
	private function _save($dataForms, &$return, $post, $submissionsData, $dataContentEmail, $nameFileByIndentifier, $requiredField, $fileAttach, $recepientEmail)
	{
		...
		if ($dataTemplates)
		{
			foreach ($dataTemplates as $emailTemplate)
			{
				...
				// AstoSoft Start
				$emailTemplate->template_subject = '['.$submission_id.'] '.$emailTemplate->template_subject;
	...
	public function _emailTemplateDefault($emailContent, $nameFileByIndentifier, $requiredField)
	{
		...
		$htmlMessage = '';
		// AstoSoft Start
		$this->_db->setQuery($this->_db->getQuery(true)->select('*')->from('#__jsn_uniform_form_pages')->where("form_id = " . (int) $this->form_id));
		...
		foreach ($pages as $key => $value_page) {
			$pagesdata = json_decode($value_page->page_content);
			...
			//foreach ($emailContent as $key => $value)
			{
				if (!empty($value->identify)) {
					$i++;
					$ident = $value->identify;
					$value = !empty($emailContent[$ident]) ? $emailContent[$ident] : ' ';
					//$value = !empty($value) ? $value : '';
					$name = !empty($nameFileByIndentifier[$ident]) ? $nameFileByIndentifier[$ident] : $ident;
					//$name = !empty($nameFileByIndentifier[$key]) ? $nameFileByIndentifier[$key] : $key;
					$required = '';
					if (isset($requiredField[$ident]) && $requiredField[$ident] == 1)
		...
		// AstoSoft
		return '<table style="border-spacing: 0;width: 650px;border: 0;font-family: Verdana;font-size: 14px;color: #39372a;"><tbody>' . $htmlMessage . '</tbody></table>';
	}
