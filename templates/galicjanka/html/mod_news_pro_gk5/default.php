...
<?php if($news_amount > 0) : ?>
	<div class="nspMain<?php if($this->config['autoanim'] == TRUE) echo ' autoanim'; ?><?php if($this->config['hover_anim'] == TRUE) echo ' hover'; ?><?php echo ' ' . $this->config['moduleclass_sfx']; ?>" id="nsp-<?php echo $this->config['module_id']; ?>" data-config="<?php echo str_replace("'links_amount': 0", "'links_amount': 0,\n'arts_current': ".$num, $news_config_json); ?>">
		<?php if(($this->config['news_column'] * $this->config['news_rows']) > 0) : ?>
			<div class="nspArts<?php echo ' '.$this->config['links_position']; ?>" style="width:<?php echo $arts_width; ?>%;">
				...
				<?php endif; ?>
				<?php if ($this->config['news_column'] == 1): $class_list = '';?>
				<div class="ramka_galeria">
				<div class="image_ramka_galeria">
				<?php else: $class_list = 'List';?>
				<?php endif;?>
				<div class="nspArtScroll1">
					<div class="nspArtScroll2 nspPages<?php echo $this->config['news_full_pages']; ?>">
					<?php for($i = 0; $i < count($news_html_tab); $i++) : if ($i == $num) $active = 'active'; else $active = '';?>
						<?php if($i == 0) : ?>
						<div class="nspArtPage<?php echo $class_list;?> <?php echo $active;?> nspCol<?php echo $this->config['news_full_pages']; ?>">
						<?php endif; ?>
							<?php 
							...
							<div class="nspArt nspCol<?php echo $this->config['news_column']; ?><?php echo (isset($news_featured_tab[$i]) && $news_featured_tab[$i] == '1') ? ' nspFeatured' : ''; ?>" style="<?php echo $style; ?>">
								<?php echo TSFilter::nosingleletter($news_html_tab[$i]);?>
							</div>
						...
						<div class="nspArtPage<?php echo $class_list;?> <?php echo $i;?> <?php echo $active;?> nspCol<?php echo $this->config['news_full_pages']; ?>">
						<?php elseif($i == count($news_html_tab) - 1) : ?>
						</div>
						<?php endif; ?>
					<?php endfor; ?>
					</div>
				<?php if ($this->config['news_column'] == 1):?>
				</div>
				</div>
				<div class="image_galeria_ramka"></div>
				<?php endif;?>
				</div>
				<div class="nspTopInterface">
					...
					<span class="nspPrev"></span>
					<span class="nspNext"></span>
					<?php endif; ?>
				</div>
			</div>
		<?php endif; ?>
