...
class NSP_GK5_com_content_View {
	// header generator
	static function header($config, $item, $index) {
		if($config['news_content_header_pos'] != 'disabled') {
			...
			$link = NSP_GK5_com_content_View::itemLink($item, $config, $index);
			//
			if($config['news_header_link'] == 1) {
				return '<h4 class="nspHeader'.$class.'"><a href="'.$link.'" title="'.htmlspecialchars($item['title']).'" onfocus="blur()">'.$output.'</a></h4>';	
			} else {
	...
	static function text($config, $item, $readmore) {
		if($config['news_content_text_pos'] != 'disabled') {
			...
			//
			$item['text'] = ($config['news_text_link'] == 1) ? '<a href="'.$link.'" onfocus="blur()">'.$item['text'].'</a>' : $item['text']; 
	...
	static function image($config, $item, $only_url = false, $pm = false, $index){		
		if($config['news_content_image_pos'] != 'disabled' || $pm) {
			$item['title'] = str_replace('"', "&quot;", $item['title']);
			$IMG_SOURCE = '';
			$IMG_LINK = NSP_GK5_com_content_View::itemLink($item, $config, $index);
			//
				...
				if($IMG_SOURCE != '') {
					...
					if($config['news_image_link'] == 1) {
						if($config['news_image_modal'] == 1) {
							return ($config['news_content_image_pos'] == 'center') ? '<div class="center'.$class.'"><a href="'.$full_size_img .'" class="modal nspImageWrapper'.$class.'"'.$margins.' onfocus="blur()"><img class="nspImage" src="'.$IMG_SOURCE.'" alt="'.htmlspecialchars($item['title']).'" style="'.$size.'"  /></a></div>' : '<a href="'.$full_size_img .'" class="modal nspImageWrapper'.$class.'"'.$margins.' onfocus="blur()"><img class="nspImage'.$class.'" src="'.$IMG_SOURCE.'" alt="'.htmlspecialchars($item['title']).'" style="'.$size.'"  /></a>';
						} else {
							return ($config['news_content_image_pos'] == 'center') ? '<div class="center'.$class.'"><a href="'.$IMG_LINK.'" class="nspImageWrapper'.$class.'"'.$margins.' onfocus="blur()"><img class="nspImage" src="'.$IMG_SOURCE.'" alt="'.htmlspecialchars($item['title']).'" style="'.$size.'"  /></a></div>' : '<a href="'.$IMG_LINK.'" class="nspImageWrapper'.$class.'"'.$margins.' onfocus="blur()"><img class="nspImage'.$class.'" src="'.$IMG_SOURCE.'" alt="'.htmlspecialchars($item['title']).'" style="'.$size.'"  /></a>';
						}
					} else {
	...
	static function readMore($config, $item) {
		...
		if($config['news_content_readmore_pos'] != 'disabled') {
			...
			//
			if($config['news_content_readmore_pos'] == 'after') {
				return '<a class="readon inline" href="'.$link.'" onfocus="blur()">'.((trim($config['readmore_text']) != '') ? $config['readmore_text'] : JText::_('MOD_NEWS_PRO_GK5_NSP_READMORE')).'</a>';
			} else {
				return '<a class="readon '.$class.'" href="'.$link.'" onfocus="blur()">'.((trim($config['readmore_text']) != '') ? $config['readmore_text'] : JText::_('MOD_NEWS_PRO_GK5_NSP_READMORE')).'</a>';
			}
		} else {
			return '';
		}
	...
	static function info($config, $item, $num = 1) {
		...
		if(
			...
			//	
			$info_category = ($config['category_link'] == 1) ? '<a href="'. NSP_GK5_com_content_View::categoryLink($item) .'" onfocus="blur()">'.$item['catname'].'</a>' : $item['catname'];
			...
			if(isset($item['tags'])) {
				...
				foreach($item['tags'] as $tag => $tag_id) {
					...
					if($i == 0) {
						$info_tags .= '<a href="' . $link . '" onfocus="blur()">' . $tag . '</a>';
					} else {
						$info_tags .= ', <a href="' . $link . '" onfocus="blur()">' . $tag . '</a>';
					}
					//
		...
		if($config['news_short_pages'] > 0) {
			...
			if($config['list_title_limit'] > 0) {
				...
				if(JString::strlen($title) > 0) {
					$title = '<h4><a href="'.$link.'" title="'.htmlspecialchars($item['title']).'" onfocus="blur()">'.$title.'</a></h4>';
				}
			}
	...
	static function itemLink($item, $config = false, $index = 0) {
		if (!empty($config['module_unique_id'])) return JRoute::_('index.php?Itemid='.$config['module_unique_id'].'&num='.$index);
		else return ($item['id'] != 0) ? JRoute::_(ContentHelperRoute::getArticleRoute($item['id'], $item['cid'])) : JRoute::_('index.php?option=com_users&view=login');
	}
