		...
		for($i = 0; $i < count($content); $i++) {	
			if($i < ($config['news_column'] * $config['news_rows'] * $config['news_full_pages'])) {
		        ...
		        if($config['use_own_article_format'] == 1) {
					...
					// GENERATING HEADER
					// AstoSoft
					$news_header = NSP_GK5_com_content_View::header($config, $content[$i], $i);
					// GENERATING IMAGE
					// Astosoft
					$news_image = NSP_GK5_com_content_View::image($config, $content[$i], false, false, $i);
					// GENERATING READMORE
					...
					for($j = 1; $j < 10; $j++) {
						...
						// Astosoft             
						if($config['news_header_order'] == $j) $generated_content_header = $news_header;
						if($config['news_image_order'] == $j)  $generated_content .= $news_image;
						// AstoSoft
						if($config['news_text_order'] == $j)   $generated_content_header .= $news_text;
					...
					// AstoSoft
					if (!empty($generated_content_header)) $generated_content .= '<div class="cont-content"><table><tr><td>'.$generated_content_header.'</td></tr></table></div>';
